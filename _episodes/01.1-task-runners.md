...
A noxfile is valid Python, so we import nox. The session decorator `@nox.session()` tells nox
...
If you have a `pyproject.toml` file, you can leverage that to install the dependencies:
```python
# noxfile.py
import nox
...
This adds a type annotation to the session object, so that IDEs and type
...
nox > python -m pip install pytest numpy
nox > pytest
==================================== test session starts ====================================
